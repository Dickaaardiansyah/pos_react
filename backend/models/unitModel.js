...
const unitModel = {
  ...
  findByProductId(productId) {
    return query(
      `SELECT pu.id, pu.product_id, pu.unit_id, pu.conversion_qty,
              pu.price, pu.price_wholesale, pu.min_qty_wholesale, u.name AS unit_name
  ...
  insertProductUnit(
    ...
    const sql =
      "INSERT INTO product_units (product_id, unit_id, conversion_qty, price, price_wholesale, min_qty_wholesale) VALUES (?, ?, ?, ?, ?, ?)";
    const params = [
      productId,
      unitId,
      conversionQty,
