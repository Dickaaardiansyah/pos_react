...
export const EMPTY_PRODUCT_FORM = {
  ...
  is_active: 1,
  additional_units: [
    { unit_id: null, unit_name: "", conversion_qty: "", price: "", price_wholesale: "", min_qty_wholesale: "" },
  ],
  variants: [
    { name: "", price: "", price_wholesale: "", min_qty_wholesale: "", barcode: "" },
  ],
};

function generateBarcodeCode() {
  const ts = Math.floor(Date.now() / 100).toString().slice(-10);
...
export function useProducts() {
  ...
  const loading = productsQuery.isLoading || categoriesQuery.isLoading || unitsQuery.isLoading;
  ...
  const filtered = products.filter((p) => {
    if (search && !p.name.toLowerCase().includes(search.toLowerCase()) && !(p.barcode || "").includes(search))
      return false;
  ...
  async function updateStock(product, { quantity, type, notes }) {
    ...
    try {
      await productsApi.updateStock(product.id, { quantity: parseInt(quantity), type, notes });
...
export function useProductForm(editProduct, onSuccess, onClose) {
  const [form, setForm] = useState(() =>
    editProduct
      ? {
          ...EMPTY_PRODUCT_FORM,
          ...editProduct,
          stock: editProduct.stock === "" || editProduct.stock == null ? "" : Number(editProduct.stock),
          min_stock: editProduct.min_stock === "" || editProduct.min_stock == null ? "" : Number(editProduct.min_stock),
          lead_time_value:
            editProduct.lead_time_value === "" || editProduct.lead_time_value == null
              ? ""
              : Number(editProduct.lead_time_value),
          safety_stock_value:
            editProduct.safety_stock_value === "" || editProduct.safety_stock_value == null
              ? ""
          ...
          additional_units: [
            ...(editProduct.additional_units || []).map((u) => ({
              ...
              conversion_qty: u.conversion_qty === "" || u.conversion_qty == null ? "" : Number(u.conversion_qty),
              price: u.price ?? "",
  ...
  useEffect(() => {
    ...
    debounceRef.current = setTimeout(async () => {
      try {
        ...
        setBarcodeStatus(res.data?.id && res.data.id === (editProduct?.id ?? null) ? "ok" : "duplicate");
      } catch (e) {
        setBarcodeStatus(e.message?.includes("404") || e.message?.includes("tidak ditemukan") ? "ok" : "error");
      }
    }, 500);
  ...
  function unitNameTaken(name, { exceptIndex } = {}) {
    ...
    return form.additional_units.some(
      (row, i) => i !== exceptIndex && (row.unit_name || "").trim().toLowerCase() === target,
    );
  }
  ...
  function ensureTrailingEmptyRow(rows) {
    ...
    if (!last || last.unit_id) {
      return [...rows, { unit_id: null, unit_name: "", conversion_qty: "", price: "", price_wholesale: "", min_qty_wholesale: "" }];
    }
    return rows;
  }
  ...
  function selectBaseUnit(option) {
    if (unitNameTaken(option.name)) {
      toast.error(`Satuan "${option.name}" sudah dipakai sebagai satuan tambahan`);
      return;
    }
  ...
  function addUnitRow() {
    setForm((f) => ({
      ...f,
      additional_units: [...f.additional_units, { unit_id: null, unit_name: "", conversion_qty: "", price: "", price_wholesale: "", min_qty_wholesale: "" }],
    }));
  }
  ...
  function selectAdditionalUnit(index, option) {
    ...
    setForm((f) => ({
      ...
      additional_units: ensureTrailingEmptyRow(
        f.additional_units.map((row, i) => (i === index ? { ...row, unit_id: option.id, unit_name: option.name } : row)),
      ),
    }));
  }
  ...
  function updateUnitRow(index, patch) {
    setForm((f) => ({
      ...f,
      additional_units: f.additional_units.map((row, i) => (i === index ? { ...row, ...patch } : row)),
    }));
  }
  ...
  function removeUnitRow(index) {
    setForm((f) => ({ ...f, additional_units: f.additional_units.filter((_, i) => i !== index) }));
  }
  ...
  function ensureTrailingVariantRow(rows) {
    const last = rows[rows.length - 1];
    const needs = !last || (last.name && last.name.trim()) || (last.price !== "" && last.price != null);
    if (!needs) return rows;
    return [...rows, { name: "", price: "", price_wholesale: "", min_qty_wholesale: "", barcode: "" }];
  }

  function updateVariantRow(index, patch) {
    setForm((f) => ({
      ...f,
      variants: ensureTrailingVariantRow(f.variants.map((row, i) => (i === index ? { ...row, ...patch } : row))),
    }));
  }
  ...
  function addVariantRow() {
    setForm((f) => ({
      ...f,
      variants: [...f.variants, { name: "", price: "", price_wholesale: "", min_qty_wholesale: "", barcode: "" }],
    }));
  }
  ...
  async function submit() {
    ...
    if (barcodeStatus === "duplicate") {
      toast.error("Barcode sudah digunakan produk lain, silakan ganti atau generate ulang");
      setActiveErrorTab("umum");
      return;
    }
    ...
    const invalidRow = form.additional_units.find(
      (row) => row.unit_name && (!row.unit_id || !row.conversion_qty || Number(row.conversion_qty) <= 0),
    );
    if (invalidRow) {
      toast.error(`Isi nilai konversi satuan "${invalidRow.unit_name || "-"}" dengan benar`);
      setActiveErrorTab("opsi");
      return;
    }

    const missingPriceRow = form.additional_units.find((row) => row.unit_id && (!row.price || Number(row.price) <= 0));
    if (missingPriceRow) {
      toast.error(`Isi harga jual satuan "${missingPriceRow.unit_name}" terlebih dahulu`);
      setActiveErrorTab("opsi");
      return;
    }
    ...
    const names = [
      ...
      ...form.additional_units.filter((r) => r.unit_id).map((r) => (r.unit_name || "").trim().toLowerCase()),
    ];
    const dupe = names.find((n, i) => names.indexOf(n) !== i);
    if (dupe) {
      toast.error(`Satuan "${dupe}" dipakai lebih dari satu kali, mohon perbaiki`);
      setActiveErrorTab("opsi");
      return;
    }
    ...
    if (form.price_wholesale && Number(form.price_wholesale) > 0) {
      if (!form.min_qty_wholesale || Number(form.min_qty_wholesale) < 2) {
        toast.error(`Isi jumlah beli minimum grosir untuk satuan dasar "${form.unit}" (minimal 2)`);
        setActiveErrorTab("harga");
        return;
      }
    }
    ...
    const invalidWholesaleRow = form.additional_units.find(
      (row) => row.unit_id && row.price_wholesale && Number(row.price_wholesale) > 0 && (!row.min_qty_wholesale || Number(row.min_qty_wholesale) < 2),
    );
    if (invalidWholesaleRow) {
      toast.error(`Isi jumlah beli minimum grosir untuk satuan "${invalidWholesaleRow.unit_name}" (minimal 2)`);
      setActiveErrorTab("opsi");
      return;
    }

    const filledVariants = form.variants.filter((v) => (v.name || "").trim() && Number(v.price) > 0);
    const filledUnits = form.additional_units.filter((row) => row.unit_id && row.conversion_qty);
    if (filledVariants.length > 0 && filledUnits.length > 0) {
      toast.error("Isi salah satu saja: Satuan tambahan (mis. beras ¼ kg) ATAU Varian (mis. Aqua Es/Biasa)");
      setActiveErrorTab("opsi");
      return;
    }

    for (const v of filledVariants) {
      if (v.price_wholesale && Number(v.price_wholesale) > 0 && (!v.min_qty_wholesale || Number(v.min_qty_wholesale) < 2)) {
        toast.error(`Isi jumlah beli minimum grosir untuk varian "${v.name}" (minimal 2)`);
        setActiveErrorTab("opsi");
        return;
      }
    }
    ...
    const payload = {
      ...form,
      price_wholesale: form.price_wholesale === "" ? null : form.price_wholesale,
      min_qty_wholesale: form.min_qty_wholesale === "" ? null : form.min_qty_wholesale,
      additional_units: form.additional_units
        ...
        .map((row) => ({
          ...
          price: row.price,
          price_wholesale: row.price_wholesale === "" ? null : row.price_wholesale,
          min_qty_wholesale: row.min_qty_wholesale === "" ? null : row.min_qty_wholesale,
        })),
      variants: form.variants
        ...
        .map((v) => ({
          ...
          min_qty_wholesale: v.min_qty_wholesale === "" ? null : v.min_qty_wholesale,
    ...
    try {
      ...
      toast.error(e.message?.toLowerCase().includes("barcode") ? "Barcode sudah digunakan — coba generate ulang" : e.message);
    } finally {
      setSubmitting(false);
    }
  }
